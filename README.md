...
Sensor data pipeline on Raspberry Pi 5 (k3s). Arduino (DHT11 + LDR) over UART, and BLE scale → JSON-RPC serial bridge → agent pods → Mosquitto MQTT.

## Structure

- `agents/` — one pod per sensor (dht11, ldr, ir, scale) plus serial-bridge
